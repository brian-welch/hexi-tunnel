...
let iterations = Math.floor(Math.random()*300) + 250, animationSpeed = 35;
...
Shapes[1] = (index, r, g, b) => {
    return `<svg id="hexagon_${index+1}" class="" style="height:${iterations - (index+1)}vh; transform: rotate(-${index*1}deg); color: rgba(${Math.floor(r - (index * (r/iterations)))},${Math.floor(g - (index * (g/iterations)))},${Math.floor(b - (index * (b/iterations)))},1)" viewBox="0 0 1199.9996 1199.9996" height="1199.9996mm" width="1199.9996mm">
    <g transform="translate(323.03455,323.03455)" style="display:inline" id="layer2">
        ...
        style="fill:currentColor;stroke-width:1;stroke-miterlimit:4;stroke-dasharray:none;stroke:rgba(${r},${g},${b},0.25);stroke-opacity:1" />
    </g>    
</svg>`;
}

Shapes[2] = (index, r, g, b) => {
    return `<svg id="hexagon_${index+1}" class="" style="height:${iterations - (index+1)}vh; transform: rotate(-${index*1}deg); color: rgba(${Math.floor(r - (index * (r/iterations)))},${Math.floor(g - (index * (g/iterations)))},${Math.floor(b - (index * (b/iterations)))},1)" viewBox="0 0 1199.9996 1199.9996" height="1199.9996mm" width="1199.9996mm">
...
const buildTunnel = () => {
    red = (150 + Math.floor(Math.random()*105)), green = (150 + Math.floor(Math.random()*105)), blue = (150 + Math.floor(Math.random()*105));
    let chooser = Math.floor(Math.random()*6) + 1;
    for (let i = 0; i < iterations; i++) {
        setTimeout(function() {
            shapesContainer.prepend(Shapes[chooser](i, red, green, blue));
        }, i * animationSpeed);
    }
}

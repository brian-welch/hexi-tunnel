...
let iterations = Math.floor(Math.random()*300) + 260, animationSpeed = 35;
let r = 50, g = 200, b = 200, dir = ['-', ''], i;
...
Shapes[1] = (index, args) => {
    return `<svg id="hexagon_${index+1}" class="hide-me" style="${widthOrHeight[aspect][0]}:${260 - ((index*(260/iterations))+1)}${widthOrHeight[aspect][1]}; transform: rotate(${spin_dir}${index*(1.30-rotator)}deg); color: rgba(${args.r_1 - (index * ((args.r_1 - args.r_2)/iterations))},${args.g_1 - (index * ((args.g_1 - args.g_2)/iterations))},${args.b_1 - (index * ((args.b_1 - args.b_2)/iterations))},1)" viewBox="0 0 1199.9996 1199.9996" height="1199.9996mm" width="1199.9996mm">
    <g transform="translate(323.03455,323.03455)" style="display:inline" id="layer2">
        ...
        style="fill:currentColor;stroke-width:0.25;stroke-miterlimit:4;stroke-dasharray:none;stroke:rgba(${255},${255},${255},0.75);stroke-opacity:1" />
    </g>    
</svg>`;
}
...
const buildTunnel = () => {
    let args = {};
    args.r_1 = (100 + Math.floor(Math.random()*75)),
        args.g_1 = (100 + Math.floor(Math.random()*75)),
        args.b_1 = (100 + Math.floor(Math.random()*75));
        args.r_2 = (75 + Math.floor(Math.random()*50)),
        args.g_2 = (75 + Math.floor(Math.random()*50)),
        args.b_2 = (75 + Math.floor(Math.random()*50));
    let chooser = Math.floor(Math.random()*6) + 1;
    $('body').css('background', `rgb(${args.r_2}, ${args.g_2}, ${args.b_2})`)
    for (let i = 0; i < iterations; i++) {
            shapesContainer.prepend(Shapes[chooser](i, args));
    }
}
